<?php

/* C:\xampp\htdocs\portalLiga\siteLiga/themes/zanor-zanor-mdb-loaded/pages/blog-homepage.htm */
class __TwigTemplate_777a5c0dce79528ee2ea71dec0447c4f898da651dddc3ad3aeaf11970a60c434 extends Twig_Template
{
    private $source;

    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->source = $this->getSourceContext();

        $this->parent = false;

        $this->blocks = array(
        );
    }

    protected function doDisplay(array $context, array $blocks = array())
    {
        // line 1
        echo "<main>
    <!--Main layout-->
    <div class=\"container\">
        <div class=\"row\">

            <!--Main column-->
            <div class=\"col-lg-8\">

                <!--Post-->
                <div class=\"post-wrapper wow fadeIn\" data-wow-delay=\"0.2s\">
                    <!--Post data-->
                    <h1 class=\"h1-responsive dark-grey-text font-bold\">Como bater Alto Crescimento em 5 semanas
                        <small class=\"text-muted\">Case</small>
                    </h1>
                    <h6>Escrito por
                        <a href=\"\">
                            <strong>Rubinho</strong>
                        </a>, 15/12/2018</h6>

                    <br>

                    <!--Featured image -->
                    <div class=\"view overlay hm-white-light z-depth-1-half rounded\">
                        <img src=\"";
        // line 24
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/liga/LogoPorte.png");
        echo "\" class=\"img-fluid\" alt=\"\">
                        <div class=\"mask\">
                        </div>
                    </div>

                    <br>

                    <!--Post excerpt-->
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, officia omnis. Vero nihil neque dignissimos
                        hic voluptas quisquam amet porro, similique libero ullam veritatis tempora cumque voluptates
                        harum. Repellendus, impedit.</p>

                    <!--\"Read more\" button-->
                    <button class=\"btn btn-blue btn-md\">Leia mais</button>
                </div>
                <!--/.Post-->

                <hr class=\"mt-5\">

                <!--Post-->
                <div class=\"post-wrapper wow fadeIn\" data-wow-delay=\"0.4s\">
                    <!--Post data-->
                    <h1 class=\"h1-responsive dark-grey-text font-bold mt-5\">Decola, o evento imersivo da Liga
                        <small class=\"text-muted\">Cobertura</small>
                    </h1>
                    <h6>Escrito por
                        <a href=\"\">
                            <strong>Rubinho</strong>
                        </a>, 07/12/2018</h6>

                    <br>

                    <!--Featured image -->
                    <div class=\"view overlay hm-white-light z-depth-1-half rounded\">
                        <img src=\"";
        // line 58
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/liga/decola.jpg");
        echo "\" class=\"img-fluid \" alt=\"\">
                        <div class=\"mask\">
                        </div>
                    </div>

                    <br>

                    <!--Post excerpt-->
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, officia omnis. Vero nihil neque dignissimos
                        hic voluptas quisquam amet porro, similique libero ullam veritatis tempora cumque voluptates
                        harum. Repellendus, impedit.</p>

                    <!--\"Read more\" button-->
                    <button class=\"btn btn-blue btn-md\">Leia mais</button>
                </div>
                <!--/.Post-->

                <hr class=\"mt-5\">

                <!--Post-->
                <div class=\"post-wrapper wow fadeIn\" data-wow-delay=\"0.6s\">
                    <!--Post data-->
                    <h1 class=\"h1-responsive font-bold dark-grey-text mt-5\">Post title
                        <small class=\"text-muted\">Secondary text</small>
                    </h1>
                    <h6>Written by
                        <a href=\"\">
                            <strong>John Doe</strong>
                        </a>, 30.04.2016</h6>

                    <br>

                    <!--Featured image -->
                    <div class=\"view overlay hm-white-light z-depth-1-half rounded\">
                        <img src=\"https://mdbootstrap.com/img/Photos/Others/images/77.jpg\" class=\"img-fluid\" alt=\"\">
                        <div class=\"mask\">
                        </div>
                    </div>

                    <br>

                    <!--Post excerpt-->
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, officia omnis. Vero nihil neque dignissimos
                        hic voluptas quisquam amet porro, similique libero ullam veritatis tempora cumque voluptates
                        harum. Repellendus, impedit.</p>

                    <!--\"Read more\" button-->
                    <button class=\"btn btn-blue btn-md\">Read more</button>
                </div>
                <!--/.Post-->

                <hr class=\"mt-5\">

                <!--Pagination-->
                <nav>
                    <ul class=\"pagination d-flex justify-content-center pg-primary wow fadeIn\" data-wow-delay=\"0.2s\">
                        <!--Arrow left-->
                        <li class=\"page-item\">
                            <a class=\"page-link\" aria-label=\"Previous\">
                                <span aria-hidden=\"true\">&laquo;</span>
                                <span class=\"sr-only\">Previous</span>
                            </a>
                        </li>

                        <!--Numbers-->
                        <li class=\"page-item active\">
                            <a class=\"page-link\">1</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">2</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">3</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">4</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">5</a>
                        </li>

                        <!--Arrow right-->
                        <li class=\"page-item\">
                            <a class=\"page-link\" aria-label=\"Next\">
                                <span aria-hidden=\"true\">&raquo;</span>
                                <span class=\"sr-only\">Next</span>
                            </a>
                        </li>
                    </ul>
                </nav>
                <!--/.Pagination-->

            </div>

            <!--Sidebar-->
            <div class=\"col-lg-4\">

                <div class=\"widget-wrapper wow fadeIn\" data-wow-delay=\"0.4s\">
                    <h4 class=\"font-bold dark-grey-text\">Categorias:</h4>
                    <br>
                    <div class=\"list-group\">
                        <a href=\"#\" class=\"list-group-item active\">Artigos</a>
                        <a href=\"#\" class=\"list-group-item\">Cases</a>
                        <a href=\"#\" class=\"list-group-item\">Entrevistas</a>
                        <a href=\"#\" class=\"list-group-item\">Novidades</a>
                        <a href=\"#\" class=\"list-group-item\">Coberturas</a>
                    </div>
                </div>

                <div class=\"widget-wrapper wow fadeIn\" data-wow-delay=\"0.6s\">
                    <h4 class=\"font-bold dark-grey-text\">Newsletter</h4>
                    <br>
                    <div class=\"card\">
                        <div class=\"card-body\">
                            <p>
                                <strong>Assine a Newsletter da Liga!</strong>
                            </p>
                            <p>Receba nossas novidades e notícias semanalmente!</p>
                            <div class=\"md-form\">
                                <i class=\"fa fa-user prefix grey-text\"></i>
                                <input type=\"text\" id=\"form1\" class=\"form-control\">
                                <label for=\"form1\">Seu nome</label>
                            </div>
                            <div class=\"md-form\">
                                <i class=\"fa fa-envelope prefix grey-text\"></i>
                                <input type=\"text\" id=\"form2\" class=\"form-control\">
                                <label for=\"form2\">Seu email</label>
                            </div>
                            <button class=\"btn btn-blue btn-md\">Assinar</button>

                        </div>
                    </div>
                </div>
                
                <!-- Archieve
                <section class=\"wow fadeIn\" data-wow-delay=\"0.4s\">
                    <h4 class=\"font-bold dark-grey-text mb-3\">Archieve</h4>
                    <!--List
                    <ul class=\"list mt-3\">
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">August 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">July 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">June 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">May 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">April 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">March 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"mb-4\">
                                <a href=\"#!\" class=\"grey-text\">Febuary 2016</a>
                            </h6>
                        </li>
                    </ul>
                    <!--List

                    <hr>

                </section>
                -->
                <!-- Archieve -->

            </div>
            <!--/.Sidebar-->
        </div>
    </div>
    <!--/.Main layout-->
</main>";
    }

    public function getTemplateName()
    {
        return "C:\\xampp\\htdocs\\portalLiga\\siteLiga/themes/zanor-zanor-mdb-loaded/pages/blog-homepage.htm";
    }

    public function isTraitable()
    {
        return false;
    }

    public function getDebugInfo()
    {
        return array (  85 => 58,  48 => 24,  23 => 1,);
    }

    public function getSourceContext()
    {
        return new Twig_Source("<main>
    <!--Main layout-->
    <div class=\"container\">
        <div class=\"row\">

            <!--Main column-->
            <div class=\"col-lg-8\">

                <!--Post-->
                <div class=\"post-wrapper wow fadeIn\" data-wow-delay=\"0.2s\">
                    <!--Post data-->
                    <h1 class=\"h1-responsive dark-grey-text font-bold\">Como bater Alto Crescimento em 5 semanas
                        <small class=\"text-muted\">Case</small>
                    </h1>
                    <h6>Escrito por
                        <a href=\"\">
                            <strong>Rubinho</strong>
                        </a>, 15/12/2018</h6>

                    <br>

                    <!--Featured image -->
                    <div class=\"view overlay hm-white-light z-depth-1-half rounded\">
                        <img src=\"{{ 'assets/images/liga/LogoPorte.png'|theme }}\" class=\"img-fluid\" alt=\"\">
                        <div class=\"mask\">
                        </div>
                    </div>

                    <br>

                    <!--Post excerpt-->
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, officia omnis. Vero nihil neque dignissimos
                        hic voluptas quisquam amet porro, similique libero ullam veritatis tempora cumque voluptates
                        harum. Repellendus, impedit.</p>

                    <!--\"Read more\" button-->
                    <button class=\"btn btn-blue btn-md\">Leia mais</button>
                </div>
                <!--/.Post-->

                <hr class=\"mt-5\">

                <!--Post-->
                <div class=\"post-wrapper wow fadeIn\" data-wow-delay=\"0.4s\">
                    <!--Post data-->
                    <h1 class=\"h1-responsive dark-grey-text font-bold mt-5\">Decola, o evento imersivo da Liga
                        <small class=\"text-muted\">Cobertura</small>
                    </h1>
                    <h6>Escrito por
                        <a href=\"\">
                            <strong>Rubinho</strong>
                        </a>, 07/12/2018</h6>

                    <br>

                    <!--Featured image -->
                    <div class=\"view overlay hm-white-light z-depth-1-half rounded\">
                        <img src=\"{{'assets/images/liga/decola.jpg'|theme }}\" class=\"img-fluid \" alt=\"\">
                        <div class=\"mask\">
                        </div>
                    </div>

                    <br>

                    <!--Post excerpt-->
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, officia omnis. Vero nihil neque dignissimos
                        hic voluptas quisquam amet porro, similique libero ullam veritatis tempora cumque voluptates
                        harum. Repellendus, impedit.</p>

                    <!--\"Read more\" button-->
                    <button class=\"btn btn-blue btn-md\">Leia mais</button>
                </div>
                <!--/.Post-->

                <hr class=\"mt-5\">

                <!--Post-->
                <div class=\"post-wrapper wow fadeIn\" data-wow-delay=\"0.6s\">
                    <!--Post data-->
                    <h1 class=\"h1-responsive font-bold dark-grey-text mt-5\">Post title
                        <small class=\"text-muted\">Secondary text</small>
                    </h1>
                    <h6>Written by
                        <a href=\"\">
                            <strong>John Doe</strong>
                        </a>, 30.04.2016</h6>

                    <br>

                    <!--Featured image -->
                    <div class=\"view overlay hm-white-light z-depth-1-half rounded\">
                        <img src=\"https://mdbootstrap.com/img/Photos/Others/images/77.jpg\" class=\"img-fluid\" alt=\"\">
                        <div class=\"mask\">
                        </div>
                    </div>

                    <br>

                    <!--Post excerpt-->
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eos, officia omnis. Vero nihil neque dignissimos
                        hic voluptas quisquam amet porro, similique libero ullam veritatis tempora cumque voluptates
                        harum. Repellendus, impedit.</p>

                    <!--\"Read more\" button-->
                    <button class=\"btn btn-blue btn-md\">Read more</button>
                </div>
                <!--/.Post-->

                <hr class=\"mt-5\">

                <!--Pagination-->
                <nav>
                    <ul class=\"pagination d-flex justify-content-center pg-primary wow fadeIn\" data-wow-delay=\"0.2s\">
                        <!--Arrow left-->
                        <li class=\"page-item\">
                            <a class=\"page-link\" aria-label=\"Previous\">
                                <span aria-hidden=\"true\">&laquo;</span>
                                <span class=\"sr-only\">Previous</span>
                            </a>
                        </li>

                        <!--Numbers-->
                        <li class=\"page-item active\">
                            <a class=\"page-link\">1</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">2</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">3</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">4</a>
                        </li>
                        <li class=\"page-item\">
                            <a class=\"page-link\">5</a>
                        </li>

                        <!--Arrow right-->
                        <li class=\"page-item\">
                            <a class=\"page-link\" aria-label=\"Next\">
                                <span aria-hidden=\"true\">&raquo;</span>
                                <span class=\"sr-only\">Next</span>
                            </a>
                        </li>
                    </ul>
                </nav>
                <!--/.Pagination-->

            </div>

            <!--Sidebar-->
            <div class=\"col-lg-4\">

                <div class=\"widget-wrapper wow fadeIn\" data-wow-delay=\"0.4s\">
                    <h4 class=\"font-bold dark-grey-text\">Categorias:</h4>
                    <br>
                    <div class=\"list-group\">
                        <a href=\"#\" class=\"list-group-item active\">Artigos</a>
                        <a href=\"#\" class=\"list-group-item\">Cases</a>
                        <a href=\"#\" class=\"list-group-item\">Entrevistas</a>
                        <a href=\"#\" class=\"list-group-item\">Novidades</a>
                        <a href=\"#\" class=\"list-group-item\">Coberturas</a>
                    </div>
                </div>

                <div class=\"widget-wrapper wow fadeIn\" data-wow-delay=\"0.6s\">
                    <h4 class=\"font-bold dark-grey-text\">Newsletter</h4>
                    <br>
                    <div class=\"card\">
                        <div class=\"card-body\">
                            <p>
                                <strong>Assine a Newsletter da Liga!</strong>
                            </p>
                            <p>Receba nossas novidades e notícias semanalmente!</p>
                            <div class=\"md-form\">
                                <i class=\"fa fa-user prefix grey-text\"></i>
                                <input type=\"text\" id=\"form1\" class=\"form-control\">
                                <label for=\"form1\">Seu nome</label>
                            </div>
                            <div class=\"md-form\">
                                <i class=\"fa fa-envelope prefix grey-text\"></i>
                                <input type=\"text\" id=\"form2\" class=\"form-control\">
                                <label for=\"form2\">Seu email</label>
                            </div>
                            <button class=\"btn btn-blue btn-md\">Assinar</button>

                        </div>
                    </div>
                </div>
                
                <!-- Archieve
                <section class=\"wow fadeIn\" data-wow-delay=\"0.4s\">
                    <h4 class=\"font-bold dark-grey-text mb-3\">Archieve</h4>
                    <!--List
                    <ul class=\"list mt-3\">
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">August 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">July 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">June 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">May 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">April 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"\">
                                <a href=\"#!\" class=\"grey-text\">March 2016</a>
                            </h6>
                        </li>
                        <li>
                            <h6 class=\"mb-4\">
                                <a href=\"#!\" class=\"grey-text\">Febuary 2016</a>
                            </h6>
                        </li>
                    </ul>
                    <!--List

                    <hr>

                </section>
                -->
                <!-- Archieve -->

            </div>
            <!--/.Sidebar-->
        </div>
    </div>
    <!--/.Main layout-->
</main>", "C:\\xampp\\htdocs\\portalLiga\\siteLiga/themes/zanor-zanor-mdb-loaded/pages/blog-homepage.htm", "");
    }
}
