<?php

/* C:\xampp\htdocs\siteLiga/themes/kevin-timber/partials/about.htm */
class __TwigTemplate_9aa035f4405b41e61efd6f14e5a401f1e5eb155610fce307f4a6b9f4748506e7 extends Twig_Template
{
    private $source;

    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->source = $this->getSourceContext();

        $this->parent = false;

        $this->blocks = array(
        );
    }

    protected function doDisplay(array $context, array $blocks = array())
    {
        // line 1
        echo "<div class=\"line2\">
\t\t\t<div class=\"container\">
\t\t\t\t<div class=\"row Fresh\">
\t\t\t\t\t<div class=\"col-md-4 Des\">
\t\t\t\t\t\t<i class=\"fa fa-heart\"></i>
\t\t\t\t\t\t<h4>Fresh & Clean Design</h4>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor. feugiat </p>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-4 Ver\">
\t\t\t\t\t\t<i class=\"fa fa-cog\"></i>
\t\t\t\t\t\t<h4>Very Flexible</h4>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor. feugiat </p>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-4 Fully\">
\t\t\t\t\t\t<i class=\"fa fa-tablet\"></i>
\t\t\t\t\t\t<h4>Fully Responsive</h4>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor. feugiat </p>
\t\t\t\t\t</div>\t\t
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t<div class=\"container\">
\t\t\t<div class=\"row\">
\t\t\t\t<div class=\"col-md-12 wwa\">
\t\t\t\t\t<div id=\"about3\" ></div>
\t\t\t\t\t<h3>Who We Are? Meet Our Team!</h3>
\t\t\t\t\t<h4>We listen, we discuss, we advise and develop. We love to learn and use the latest technologies.</h4>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t<div class=\"container\">
\t\t\t<div class=\"row team\">
\t\t\t\t<div class=\"col-md-4 b1\">
\t\t\t\t\t\t<img class=\"img-responsive\" src=\"";
        // line 34
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/picTeam/picT1.png");
        echo "\" alt=\"pic\"/>
\t\t\t\t\t\t<h4>Tom Simpson</h4>
\t\t\t\t\t\t<h5>CEO</h5>
\t\t\t\t\t\t<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit <br/>quisque tempus ac eget diam et laoreet phasellus ut nisi id leo molestie. adipiscing vitae vel quam proin eget mauris eget.</p>
\t\t\t\t\t\t<ul>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-facebook-square\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-pinterest\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-twitter\" ></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-google-plus-square\"></i></a></li>
\t\t\t\t\t\t</ul>
\t\t\t\t</div>
\t\t\t
\t\t\t
\t\t\t\t<div class=\"col-md-4\">
\t\t\t\t\t\t<img class=\"img-responsive\" src=\"";
        // line 48
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/picTeam/picT2.png");
        echo "\" alt=\"pic\"/>
\t\t\t\t\t\t<h4>John Doe</h4>
\t\t\t\t\t\t<h5>Project Manager</h5>
\t\t\t\t\t\t<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit<br/> quisque tempus ac eget diam et laoreet phasellus ut nisi id leo molestie. adipiscing vitae vel quam proin eget mauris eget.</p>
\t\t\t\t\t\t<ul>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-facebook-square\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-pinterest\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-twitter\" ></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-google-plus-square\"></i></a></li>
\t\t\t\t\t\t</ul>
\t\t\t\t</div>
\t\t
\t\t\t
\t\t\t\t<div class=\"col-md-4 b3\">
\t\t\t\t\t\t<img class=\"img-responsive\" src=\"";
        // line 62
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/picTeam/picT3.png");
        echo "\" alt=\"pic\"/>
\t\t\t\t\t\t<h4>Anna White</h4>
\t\t\t\t\t\t<h5>Developer</h5>
\t\t\t\t\t\t<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit<br/> quisque tempus ac eget diam et laoreet phasellus ut nisi id leo molestie. adipiscing vitae vel quam proin eget mauris eget.</p>
\t\t\t\t\t\t<ul>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-facebook-square\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-pinterest\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-twitter\" ></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-google-plus-square\"></i></a></li>
\t\t\t\t\t\t</ul>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t<div class=\"container\">
\t\t\t<div class=\"row\">
\t\t\t\t<div class=\"col-md-12 hr1\">
\t\t\t\t\t<hr/>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>\t\t
\t\t<div class=\"container\">
\t\t\t<div class=\"row\">
\t\t\t\t<div class=\"col-md-3 bar\">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"64\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"64\">
\t\t\t\t\t\t\t  <span class=\"percent\">64</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>WordPress</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t\t<div class=\"col-md-3 bar\">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"22\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"22\">
\t\t\t\t\t\t\t  <span class=\"percent\">22</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>HTML5</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t\t<div class=\"col-md-3 bar \">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"84\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"22\">
\t\t\t\t\t\t\t  <span class=\"percent\">84</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>CSS3</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t\t<div class=\"col-md-3 bar \">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"45\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"45\">
\t\t\t\t\t\t\t  <span class=\"percent\">45</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>Woocommerce</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>\t
\t\t
\t\t
\t\t<div class=\"container\">
\t\t\t<div class=\"row aboutUs\">
\t\t\t\t<div class=\"col-md-12 \">
\t\t\t\t\t<h3>What Our Customers Say About Us?</h3>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t
\t\t<div style=\"position: relative;\">
\t\t
\t\t\t<div class=\"container\">
\t\t\t\t<div class=\"row about\">
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about1\">
\t\t\t\t\t\t<img class=\"pic1Ab\" src=\"";
        // line 154
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/picAbout/aboutP1.png");
        echo "\" alt=\"pic\"/>
\t\t\t\t\t\t\t<h3>Anna Smith, Company Inc.</h3>
\t\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor feugiat fermentum quisque eget pharetra, felis et venenatis. aliquam, nulla nisi lobortis elit ac.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about2\">
\t\t\t\t\t\t<img class=\"pic2Ab\" src=\"";
        // line 161
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/picAbout/aboutP2.png");
        echo "\" alt=\"pic\"/>
\t\t\t\t\t\t\t<h3>John Doe, Company Inc.</h3>
\t\t\t\t\t\t\t<p>Consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo, adipiscing auctor, ut auctor feugiat fermentum nec quisque eget pharetra, felis et venenatis aliquam, nulla nisi lobortis elit, ac luctus.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t</div>
\t\t
\t\t\t<div class=\"horL\"></div>
\t\t
\t\t\t<div class=\"container\">
\t\t\t\t<div class=\"row about\">
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about1\">
\t\t\t\t\t\t<img class=\"pic1Ab\" src=\"";
        // line 175
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/picAbout/aboutP3.png");
        echo "\" alt=\"pic\" />
\t\t\t\t\t\t\t<h3>Tom Sawyer, Company Inc.</h3>
\t\t\t\t\t\t\t<p>A auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor feugiat fermentum quisque eget pharetra, felis et venenatis aliquam, nulla nisi lobortis elit, acnterdum ante feugiat vitae.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about2\">
\t\t\t\t\t\t<img class=\"pic2Ab\" src=\"";
        // line 182
        echo $this->extensions['Cms\Twig\Extension']->themeFilter("assets/images/picAbout/aboutP4.png");
        echo "\" alt=\"pic\"/>
\t\t\t\t\t\t\t<h3>Sarah White, Company Inc.</h3>
\t\t\t\t\t\t\t<p>Ornare nibh a auctor, mauris scelerisque eu proin nec urna nec a quis justo adipiscing auctor ut auctor feugiat fermentum quisque eget pharetra felis et venenatis aliquam, nulla nisi lobortis elit, ac eleifend.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t</div>
\t\t
\t\t</div>";
    }

    public function getTemplateName()
    {
        return "C:\\xampp\\htdocs\\siteLiga/themes/kevin-timber/partials/about.htm";
    }

    public function isTraitable()
    {
        return false;
    }

    public function getDebugInfo()
    {
        return array (  224 => 182,  214 => 175,  197 => 161,  187 => 154,  92 => 62,  75 => 48,  58 => 34,  23 => 1,);
    }

    public function getSourceContext()
    {
        return new Twig_Source("<div class=\"line2\">
\t\t\t<div class=\"container\">
\t\t\t\t<div class=\"row Fresh\">
\t\t\t\t\t<div class=\"col-md-4 Des\">
\t\t\t\t\t\t<i class=\"fa fa-heart\"></i>
\t\t\t\t\t\t<h4>Fresh & Clean Design</h4>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor. feugiat </p>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-4 Ver\">
\t\t\t\t\t\t<i class=\"fa fa-cog\"></i>
\t\t\t\t\t\t<h4>Very Flexible</h4>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor. feugiat </p>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-4 Fully\">
\t\t\t\t\t\t<i class=\"fa fa-tablet\"></i>
\t\t\t\t\t\t<h4>Fully Responsive</h4>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor. feugiat </p>
\t\t\t\t\t</div>\t\t
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t<div class=\"container\">
\t\t\t<div class=\"row\">
\t\t\t\t<div class=\"col-md-12 wwa\">
\t\t\t\t\t<div id=\"about3\" ></div>
\t\t\t\t\t<h3>Who We Are? Meet Our Team!</h3>
\t\t\t\t\t<h4>We listen, we discuss, we advise and develop. We love to learn and use the latest technologies.</h4>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t<div class=\"container\">
\t\t\t<div class=\"row team\">
\t\t\t\t<div class=\"col-md-4 b1\">
\t\t\t\t\t\t<img class=\"img-responsive\" src=\"{{ 'assets/images/picTeam/picT1.png'|theme }}\" alt=\"pic\"/>
\t\t\t\t\t\t<h4>Tom Simpson</h4>
\t\t\t\t\t\t<h5>CEO</h5>
\t\t\t\t\t\t<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit <br/>quisque tempus ac eget diam et laoreet phasellus ut nisi id leo molestie. adipiscing vitae vel quam proin eget mauris eget.</p>
\t\t\t\t\t\t<ul>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-facebook-square\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-pinterest\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-twitter\" ></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-google-plus-square\"></i></a></li>
\t\t\t\t\t\t</ul>
\t\t\t\t</div>
\t\t\t
\t\t\t
\t\t\t\t<div class=\"col-md-4\">
\t\t\t\t\t\t<img class=\"img-responsive\" src=\"{{ 'assets/images/picTeam/picT2.png'|theme }}\" alt=\"pic\"/>
\t\t\t\t\t\t<h4>John Doe</h4>
\t\t\t\t\t\t<h5>Project Manager</h5>
\t\t\t\t\t\t<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit<br/> quisque tempus ac eget diam et laoreet phasellus ut nisi id leo molestie. adipiscing vitae vel quam proin eget mauris eget.</p>
\t\t\t\t\t\t<ul>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-facebook-square\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-pinterest\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-twitter\" ></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-google-plus-square\"></i></a></li>
\t\t\t\t\t\t</ul>
\t\t\t\t</div>
\t\t
\t\t\t
\t\t\t\t<div class=\"col-md-4 b3\">
\t\t\t\t\t\t<img class=\"img-responsive\" src=\"{{ 'assets/images/picTeam/picT3.png'|theme }}\" alt=\"pic\"/>
\t\t\t\t\t\t<h4>Anna White</h4>
\t\t\t\t\t\t<h5>Developer</h5>
\t\t\t\t\t\t<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit<br/> quisque tempus ac eget diam et laoreet phasellus ut nisi id leo molestie. adipiscing vitae vel quam proin eget mauris eget.</p>
\t\t\t\t\t\t<ul>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-facebook-square\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-pinterest\"></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-twitter\" ></i></a></li>
\t\t\t\t\t\t\t<li><a href=\"#\"><i class=\"fa fa-google-plus-square\"></i></a></li>
\t\t\t\t\t\t</ul>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t<div class=\"container\">
\t\t\t<div class=\"row\">
\t\t\t\t<div class=\"col-md-12 hr1\">
\t\t\t\t\t<hr/>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>\t\t
\t\t<div class=\"container\">
\t\t\t<div class=\"row\">
\t\t\t\t<div class=\"col-md-3 bar\">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"64\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"64\">
\t\t\t\t\t\t\t  <span class=\"percent\">64</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>WordPress</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t\t<div class=\"col-md-3 bar\">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"22\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"22\">
\t\t\t\t\t\t\t  <span class=\"percent\">22</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>HTML5</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t\t<div class=\"col-md-3 bar \">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"84\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"22\">
\t\t\t\t\t\t\t  <span class=\"percent\">84</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>CSS3</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t\t<div class=\"col-md-3 bar \">
\t\t\t\t\t<div class=\"progB chart\" data-percent=\"45\"  data-animate=\"3500\">
\t\t\t\t\t\t<div class=\"chart chart-content\">
\t\t\t\t\t\t\t<div class=\"percentage\" data-percent=\"45\">
\t\t\t\t\t\t\t  <span class=\"percent\">45</span>
\t\t\t\t\t\t\t</div>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"textP\">
\t\t\t\t\t\t<h3>Woocommerce</h3>
\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor <br/>mauris scelerisque eu proin nec urna quis. </p>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>\t
\t\t
\t\t
\t\t<div class=\"container\">
\t\t\t<div class=\"row aboutUs\">
\t\t\t\t<div class=\"col-md-12 \">
\t\t\t\t\t<h3>What Our Customers Say About Us?</h3>
\t\t\t\t</div>
\t\t\t</div>
\t\t</div>
\t\t
\t\t<div style=\"position: relative;\">
\t\t
\t\t\t<div class=\"container\">
\t\t\t\t<div class=\"row about\">
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about1\">
\t\t\t\t\t\t<img class=\"pic1Ab\" src=\"{{ 'assets/images/picAbout/aboutP1.png'|theme }}\" alt=\"pic\"/>
\t\t\t\t\t\t\t<h3>Anna Smith, Company Inc.</h3>
\t\t\t\t\t\t\t<p>Nulla consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor feugiat fermentum quisque eget pharetra, felis et venenatis. aliquam, nulla nisi lobortis elit ac.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about2\">
\t\t\t\t\t\t<img class=\"pic2Ab\" src=\"{{ 'assets/images/picAbout/aboutP2.png'|theme }}\" alt=\"pic\"/>
\t\t\t\t\t\t\t<h3>John Doe, Company Inc.</h3>
\t\t\t\t\t\t\t<p>Consectetur ornare nibh, a auctor mauris scelerisque eu proin nec urna quis justo, adipiscing auctor, ut auctor feugiat fermentum nec quisque eget pharetra, felis et venenatis aliquam, nulla nisi lobortis elit, ac luctus.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t</div>
\t\t
\t\t\t<div class=\"horL\"></div>
\t\t
\t\t\t<div class=\"container\">
\t\t\t\t<div class=\"row about\">
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about1\">
\t\t\t\t\t\t<img class=\"pic1Ab\" src=\"{{ 'assets/images/picAbout/aboutP3.png'|theme }}\" alt=\"pic\" />
\t\t\t\t\t\t\t<h3>Tom Sawyer, Company Inc.</h3>
\t\t\t\t\t\t\t<p>A auctor mauris scelerisque eu proin nec urna quis justo adipiscing auctor ut auctor feugiat fermentum quisque eget pharetra, felis et venenatis aliquam, nulla nisi lobortis elit, acnterdum ante feugiat vitae.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t\t<div class=\"col-md-6\">
\t\t\t\t\t\t<div class=\"about2\">
\t\t\t\t\t\t<img class=\"pic2Ab\" src=\"{{ 'assets/images/picAbout/aboutP4.png'|theme }}\" alt=\"pic\"/>
\t\t\t\t\t\t\t<h3>Sarah White, Company Inc.</h3>
\t\t\t\t\t\t\t<p>Ornare nibh a auctor, mauris scelerisque eu proin nec urna nec a quis justo adipiscing auctor ut auctor feugiat fermentum quisque eget pharetra felis et venenatis aliquam, nulla nisi lobortis elit, ac eleifend.</p>
\t\t\t\t\t\t</div>
\t\t\t\t\t</div>
\t\t\t\t</div>
\t\t\t</div>
\t\t
\t\t</div>", "C:\\xampp\\htdocs\\siteLiga/themes/kevin-timber/partials/about.htm", "");
    }
}
